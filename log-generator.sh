...
generate_log() {
  ...
  local json_log="{\"hostname\":\"$host_name\",\"app_name\":\"$random_app_name\",\"container_id\":\"$container_id\",\"log_level\":\"$random_log_level\",\"message\":\"$random_message\",\"request_url\":\"$random_request_url\",\"parameters\":\"$random_parameters\", \"date_time\":\"$date_time\"}"
  echo "$json_log" >> "/var/log/test_json/generate.json"
}
